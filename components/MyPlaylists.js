          ...
          <div className="flex gap-2 flex-row flex-wrap scroll-smooth duration-300 snap-y snap-mandatory  content-start items-start justify-around mx-auto  text-white">
            {data?.map((value, index) => {
              return (
                <div
                  key={index}
                  className="rounded-2xl hover:p-3 hover:backdrop-blur-md flex flex-col w-[95vw] flex-wrap md:max-w-md items-start h-36 md:h-48 md:p-2 p-2 bg-black hover:bg-slate-800  hover:bg-opacity-10 bg-opacity-30 duration-200 md:flex-row"
                >
                  <a
                    target="0"
                  ...
                  <div className="hover:px-1 relative p-1 flex flex-col h-full items-start justify-between w-56 duration-200">
                    <div>
                      <p className="line-clamp-2 md:line-clamp-2 text-lg w-56 md:text-2xl inline font-bold tracking-tight text-slate-900 dark:text-white">
