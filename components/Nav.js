...
const Nav = ({
  ...
  return (
    <div className='px-5  w-screen backdrop-blur-sm rounded-lg duration-300 z-10'>
      <div>
        <div className='flex justify-between items-center  py-3 md:justify-start md:space-x-5'>
          ...
          <div className='-my-2 md:hidden'>
            <button type='button' className='rounded-xl p-2 dark:text-white inline-flex items-center justify-center text-gray-400' onClick={() => setOpen(!open)}>
      ...
      {/* Mobile Nav Bar */}
      <div className={open ? 'block  absolute top-0 inset-x-0  origin-top-right md:hidden' : 'opacity-0 hidden absolute top-0 inset-x-0 origin-top-right md:hidden'}>
        <div className='h-screen z-10 '>
          <div className='shadow-lg h-full bg-gray-900 bg-opacity-90  text-white duration-300 font-bold '>
            <div className='p-3 flex items-center justify-between'>
              <Link href='/'>
                <div onClick={() => setOpen(!open)} className='flex p-1 justify-start items-center lg:w-0 lg:flex-1'>
              ...
              <div>
                <button type='button' className='text-white font-bold pr-4 inline-flex items-center justify-center' onClick={() => setOpen(!open)}>
            ...
            <div className='h-full pb-10 flex flex-col justify-center'>
              <div className='flex flex-col items-start p-1 justify-center'>
                <hi className='mx-5 font-extrabold border-b-2 border-dashed w-72 text-3xl text-green-400'>Menu</hi>
                <Link href='/Home'>
                  <h1 onClick={() => setOpen(!open)} className='duration-300 hover:text-green-400 rounded-3xl hover:p-4 my-2 m-1 p-3 text-5xl'>
                    Go to Home
                  </h1>
                </Link>
                <Link href='/Account'>
                  ...
                  <>
                    <Link href='/Account'>
                      <div className='rounded-3xl m-2 h-20  hover:bg-green-400 border-opacity-20 hover:scale-110 hover:text-green-400'>
                        <Image width={50} height={50} alt='logo' className='rounded-3xl hover:shadow-md float-left h-full' src={session?.data?.user?.image} />
                      </div>
                    </Link>
                    <div className='p-1 dark:text-gray-300'>
                      <p className='text-2xl text-green-400'>{session?.data?.user?.name}</p>
                      <p className='text-lg truncate'>{session?.data?.user?.email}</p>
                    </div>
                    <button
                      className='inline-block dark:text-white hover:text-green-400 p-1 h-8 text-sm hover:scale-105 duration-100'
                      onClick={() => {
