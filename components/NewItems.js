          ...
          <div className="flex gap-2 flex-row flex-wrap scroll-smooth duration-300 snap-y snap-mandatory  content-start items-start justify-around mx-auto  text-white">
            {data?.map((value, index) => {
              return (
                <div
                  key={index}
                  className="rounded-3xl hover:backdrop-blur-md flex flex-col hover:scale-95 w-[95vw] flex-wrap md:max-w-4xl items-start h-36 md:h-44 md:p-2 p-2 bg-slate-200 hover:bg-opacity-20 bg-opacity-10 duration-200 md:flex-row"
                >
                  <a
                    target="0"
                  ...
                  <div className="relative p-2 flex flex-col h-full items-start justify-between w-[80%] duration-200">
                    <div className="m-0.5">
                      <p className="line-clamp-2 md:line-clamp-2 text-lg md:text-2xl inline font-bold tracking-tight text-slate-900 dark:text-white">
                        <a target="0" href={value?.external_urls?.spotify}>
                          {value?.name}
                        </a>
                      </p>
                      <p className="line-clamp-1 inline  md:line-clamp-2 text-base w-96 tracking-tight font-bold text-slate-300 ">
